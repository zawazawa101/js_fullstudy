// compontents/calendar/calendar.js
Component({
  /**
   * 组件的属性列表
   */
  ...
  methods: {
    ...
    display(year, month, date) {
      ...
      this.createEmptyGirds(year, month)
    },
    ...
    createEmptyGirds(year, month){
      ...
      for(let i = 1; i <= emptyDays; i++) {
        emptyGridsBefore.push(preMonthDays - (emptyDays - i))//  上月空出的天数
      }
      // console.log(emptyGridsBefore)
      let after = (42 - thisMonthDays - emptyDays) - 7 >= 0 ? (42 - thisMonthDays - emptyDays) - 7 >= 0 : (42 - thisMonthDays - emptyDays)
